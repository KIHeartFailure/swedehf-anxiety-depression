...
coxvars_notqol <- setdiff(coxvars, qolvars)
...
outcomesfunc_inner <- function(data = rsdata, impdata = imprsdata, time, event, eventname, xvar = "shf_anxiety",
  ...
  ## 4 row - adjusted regression excluding qol variables
  out[4, 2] <- "Adjusted Hazard Ratio (95% CI), p-value excluding 6 QoL variables"
  coxvars_notqol2 <- setdiff(coxvars_notqol, novar)
  mod <- summary(pool(with(impdata, coxph(formula(paste0(
    "Surv(", time, ",", event, " == 'Yes') ~ ", xvar, "+", paste(coxvars_notqol2, collapse = " + ")
  ))))))
