...
basetabfunc <- function(data, novars = NULL) {
  tab <- data %>%
    count(shf_anxiety, .drop = F) %>%
    mutate(n = paste0(n, " (", fn(n / sum(n) * 100, catdig), ")")) %>%
    pivot_wider(names_from = shf_anxiety, values_from = n) %>%
    mutate(
      var = "N (%)",
      level = NA,
      p = NA,
    ...
    select(var, level, pmis, levels(rsdata %>% pull(shf_anxiety)), p)

  tabvars2 <- setdiff(tabvars, novars)
  tabtmp <- sapply(tabvars2,
    FUN = basetabfunc_inner, data = data,
    stratavar = "shf_anxiety", catdig = catdig, contdig = contdig
  )
  ...
  tab <- tab %>%
    mutate(
      ...
      footnote2 = if_else(!is.na(var) & var %in% c("shf_sglt2"), footnote_marker_alphabet(3, format = "latex"), "")
    ) %>%
    unite(footnote, starts_with("footnote"), sep = "")

  cn <- c("Variable", c("Missing (%)", levels(data %>% pull(shf_anxiety)), "p-value"))
  ...
  default_kable(tab,
    ...
    add_header_above(c(" " = 1, " " = 1, "Anxiety/depression" = 3, " " = 1)) %>%
    footnote(
